...
const BASE_URL = PRODUCTS_ENDPOINT;
...
function reducer(state, action) {
  switch (action.type) {
    ...
    case ACTIONS.ERROR:
      return {
        ...state, loading: false, error: action.payload.error, products: []
      };
...
export default function useFetchProducts(params, pageNumber) {
  ...
  useEffect(() => {
    ...
    axios.get(BASE_URL, {
      ...
      params: { pageNumber: pageNumber + 1, ...params }
    }).then((res) => {
      dispatch({
